#pragma once

const sf::Vector2u WIN_RES{ 720,480 };

const float FRAMERATE = 1.f / 60.f;
const unsigned NMBR_OF_PARTICLES = 450;

const std::string SPLASH_MUSIC_PATH = "./resources/audio/SPLASH_MUSIC.wav";
const std::string TRANSITION_SOUND_PATH = "./resources/audio/TRANSITION_SOUND.wav";
const std::string APPLAUSE_SOUND_PATH = "./resources/audio/APPLAUSE_SOUND.wav";

const std::string MAIN_FONT_PATH = "./resources/fonts/ethnocentric.ttf";

const std::string SPLASH_BG_PATH = "./resources/pictures/splashscreen.png";
const std::string MENU_BG_PATH = "./resources/pictures/menuscreen.png";
const std::string OPTIONS_BG_PATH = "./resources/pictures/optionsscreen.png";
const std::string KEY_BG_PATH = "./resources/pictures/keyscreen.png";
const std::string GAME_BG_PATH = "./resources/pictures/game_static.png";
const std::string PAUSED_BG_PATH = "./resources/pictures/pausedscreen.png";
const std::string END_BG_PATH = "./resources/pictures/endscreen.png";

const std::string HIT1_ON_PATH = "./resources/pictures/hit1_on.png";
const std::string HIT2_ON_PATH = "./resources/pictures/hit2_on.png";
const std::string HIT3_ON_PATH = "./resources/pictures/hit3_on.png";
const std::string HIT4_ON_PATH = "./resources/pictures/hit4_on.png";
const std::string HIT_OFF_PATH = "./resources/pictures/hit_off.png";
const std::string DOT1_PATH = "./resources/pictures/dot1.png";
const std::string DOT2_PATH = "./resources/pictures/dot2.png";
const std::string DOT3_PATH = "./resources/pictures/dot3.png";
const std::string DOT4_PATH = "./resources/pictures/dot4.png";
const std::string DOT_HIT_PATH = "./resources/pictures/dotHit.png";
const std::string EMPTY_TEX_PATH = "./resources/pictures/emptyTexture.png";
const std::string ICON_PATH = "./resources/pictures/bbIcon.ico";